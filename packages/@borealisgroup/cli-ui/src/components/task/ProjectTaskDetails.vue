    ...
    <template v-if="task">
      <div class="header">
        ...
        <div
          class="command"
          v-tooltip="
            `${$t('org.vue.views.project-task-details.command')}:<br><code>${
              task.command
            }</code>`
          "
        >
...
export default {
  ...
  metaInfo() {
    return {
      title:
        this.task &&
        `${this.task.name} - ${this.$t('org.vue.views.project-tasks.title')}`,
    };
  },
  ...
  computed: {
    currentViewComponent() {
      if (this.currentView !== '_output') {
        const view = this.task.views.find(view => view.id === this.currentView);
        if (view) {
