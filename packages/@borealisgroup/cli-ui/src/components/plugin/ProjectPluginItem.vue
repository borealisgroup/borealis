<template>
  <div class="project-plugin-item list-item">
    <div class="content">
      ...
      <ListItemInfo :name="plugin.id" :link="plugin.website" show-description>
        <span slot="description" class="plugin-description">
          <span class="info version">
            <span class="label">{{
              $t('org.vue.components.project-plugin-item.version')
            }}</span>
            <span class="value">{{
              pluginDetails && pluginDetails.version.current
            }}</span>
          </span>

          <span class="info latest">
            <span class="label">{{
              $t('org.vue.components.project-plugin-item.latest')
            }}</span>
            <VueIcon
              v-if="
                pluginDetails &&
                  pluginDetails.version.current !== pluginDetails.version.latest
              "
              icon="warning"
              class="top medium"
            />
            <span class="value">{{
              pluginDetails && pluginDetails.version.latest
            }}</span>
          </span>
          ...
          <span v-if="plugin.installed" class="info">
            <template v-if="isLocal">
              <VueIcon icon="folder" class="top medium" />
      ...
      <VueButton
        ...
        class="icon-button"
        v-tooltip="
          $t('org.vue.components.project-plugin-item.actions.refresh', {
            target: plugin.id,
          })
        "
      ...
      <VueButton
        v-else-if="
          pluginDetails &&
            pluginDetails.version.current !== pluginDetails.version.wanted
        "
        icon-left="file_download"
        class="icon-button"
        v-tooltip="
          $t('org.vue.components.project-plugin-item.actions.update', {
            target: plugin.id,
          })
        "
        :loading-left="updating"
