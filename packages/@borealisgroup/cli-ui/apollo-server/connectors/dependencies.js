...
// Utils
const { hasYarn, resolveModule } = require('@vue/cli-shared-utils');
...
function findDependencies(deps, type, file, context) {
  return Object.keys(deps)
    .filter(id => !isPlugin(id) && id !== CLI_SERVICE)
    .map(id => ({
      id,
...
function getLocalPath(id, context) {
  ...
  const projectPkg = folders.readPackage(
    projects.getCurrent(context).path,
    context,
    true
  );
