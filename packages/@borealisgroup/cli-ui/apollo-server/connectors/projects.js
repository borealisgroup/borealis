...
const Creator = require('@borealisgroup/cli/lib/Creator');
const { getPromptModules } = require('@borealisgroup/cli-config/promptModules');
const { getFeatures } = require('@borealisgroup/cli/lib/util/features');
const { defaults } = require('@borealisgroup/cli/lib/options');
...
function autoClean(projects, context) {
  ...
  if (result.length !== projects.length) {
    console.log(
      `Auto cleaned ${projects.length -
        result.length} projects (folder not found).`
    );
...
async function initCreator(context) {
  ...
  presets = [
    ...Object.keys(presetsData).map(key => {
      ...
      const info = {
        ...
        name:
          key === 'default'
            ? 'org.vue.views.project-create.tabs.presets.default-preset'
            : key,
        features,
        link: null,
        raw: preset,
  ...
  features = [
    ...
    })),
    {
      ...
      description:
        'org.vue.views.project-create.tabs.features.userConfigFiles.description',
      link: null,
      plugins: null,
      enabled: false,
    },
  ...
  if (preset) {
    for (const feature of features) {
      feature.enabled = !!(
        ...
        (feature.plugins &&
          preset.features.some(f => feature.plugins.includes(f)))
      );
    }
    if (preset.raw) {
      if (preset.raw.router) {
        await setFeatureEnabled(
          { id: 'router', enabled: true, updatePrompts: false },
          context
        );
      }
      if (preset.raw.vuex) {
        await setFeatureEnabled(
          { id: 'vuex', enabled: true, updatePrompts: false },
          context
        );
      }
      if (preset.raw.cssPreprocessor) {
        await setFeatureEnabled(
          { id: 'css-preprocessor', enabled: true, updatePrompts: false },
          context
        );
      }
      if (preset.raw.useConfigFiles) {
        await setFeatureEnabled(
          { id: 'use-config-files', enabled: true, updatePrompts: false },
          context
        );
      }
    }
...
async function create(input, context) {
  return progress.wrap(PROGRESS_ID, context, async setProgress => {
    ...
    const name = (creator.name = (inCurrent
    ...
    // Create
    const args = ['--skipGetStarted'];
    if (input.packageManager)
      args.push('--packageManager', input.packageManager);
    ...
    const child = execa('vue', ['create', name, ...args], {
      cwd: cwd.get(),
