...
const PackageManager = require('@borealisgroup/cli/lib/util/ProjectPackageManager');
...
async function list(
  file,
  context,
  { resetApi = true, lightApi = false, autoLoadApi = true } = {}
) {
...
function findPlugins(deps, file) {
  return Object.keys(deps)
    .filter(id => {
      return isPlugin(id) || id === CLI_SERVICE;
    })
    .map(id => ({
      id,
...
function runPluginApi(id, pluginApi, context, filename = 'ui') {
  ...
  if (module) {
    if (typeof module !== 'function') {
      log(
        `${chalk.red(
          'ERROR'
        )} while loading plugin API: no function exported, for`,
        name,
        chalk.grey(pluginApi.cwd)
      );
      logs.add({
        type: 'error',
  ...
  // Locales
  try {
    const folder = fs.existsSync(id)
      ? id
      : dependencies.getPath({ id, file: pluginApi.cwd });
...
async function initPrompts(id, context) {
  ...
  try {
    let data = require(path.join(
      dependencies.getPath({ id, file: cwd.get() }),
      'prompts'
    ));
...
function update({ id, full }, context) {
  return progress.wrap('plugin-update', context, async setProgress => {
    ...
    const { current, wanted, localPath } = await dependencies.getVersion(
      plugin,
      context
    );

    if (localPath) {
      await updateLocalPackage(
        { cwd: cwd.get(), id, localPath, full },
        context
      );
    } else {
      const pm = new PackageManager({ context: cwd.get() });
...
async function updateLocalPackage(
  { id, cwd, localPath, full = true },
  context
) {
...
function serveFile({ pluginId, projectId = null, file }, res) {
    ...
    const basePath =
      pluginId === '.'
        ...
        : dependencies.getPath({
            id: decodeURIComponent(pluginId),
            file: baseFile,
          });
    if (basePath) {
  ...
  res.status(404);
  res.send(
    `Addon ${pluginId} not found in loaded addons. Try opening a vue-cli project first?`
  );
}
