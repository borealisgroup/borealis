...
async function getDiffs(context) {
  ...
  for (const n in list) {
    ...
    const highlightedContentFrom =
      fromContent && highlightCode(fileDiff.from, fromContent).split('\n');
    const highlightedContentTo = highlightCode(
      fileDiff.to,
      fs.readFileSync(path.resolve(cwd.get(), fileDiff.to), {
        encoding: 'utf8',
      })
    ).split('\n');
