...
async function registerDefinition({ definition, project }, context) {
  ...
  if (!definition.icon) {
    ...
    const plugin = plugins.findOne(
      { id: definition.pluginId, file: cwd.get() },
      context
    );
...
function findValidPosition(definition, currentWidget = null) {
  ...
  const width =
    (currentWidget && currentWidget.width) ||
    definition.defaultWidth ||
    definition.minWidth;
  const height =
    (currentWidget && currentWidget.height) ||
    ...
    for (const otherWidget of widgets) {
      if (otherWidget !== widget) {
        if (areOverlapping(otherWidget, widget)) {
          ...
          Object.assign(
            otherWidget,
            findValidPosition(otherDefinition, otherWidget)
          );
        }
      }
    }
