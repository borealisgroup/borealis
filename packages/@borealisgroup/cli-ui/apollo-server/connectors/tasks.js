...
async function list({ file = null, api = true } = {}, context) {
  ...
  if (pkg.scripts) {
    ...
    let currentTasks = scriptKeys.map(name => {
      const id = `${file}:${name}`;
    ...
    if (api && pluginApi) {
      currentTasks = currentTasks.concat(
        plugins.getApi(file).addedTasks.map(task => {
      ...
      addLog(
        {
          ...
          type: 'stdout',
          text: chalk.red(
            `Error while running task ${task.id} with message '${error.message}'`
          ),
        },
        context
      );
  ...
  if (task) {
    ...
    if (task.prompts.length) {
      prompts.add({
        ...
        description:
          'org.vue.views.project-task-details.override-args.description',
      });
    }
