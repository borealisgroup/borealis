...
const resolvers = [
  {
    ...
    Mutation: {
      fileOpenInEditor: (root, { input }, context) =>
        files.openInEditor(input, context),
    ...
    Subscription: {
      ...
      progressChanged: {
        subscribe: withFilter(
          // Iterator
          (parent, args, { pubsub }) =>
            pubsub.asyncIterator(channels.PROGRESS_CHANGED),
          // Filter
      ...
      progressRemoved: {
        subscribe: withFilter(
          // Iterator
          (parent, args, { pubsub }) =>
            pubsub.asyncIterator(channels.PROGRESS_REMOVED),
          // Filter
      ...
      clientAddonAdded: {
        subscribe: (parent, args, { pubsub }) =>
          pubsub.asyncIterator(channels.CLIENT_ADDON_ADDED),
      },
      sharedDataUpdated: {
        subscribe: withFilter(
          (parent, args, { pubsub }) =>
            pubsub.asyncIterator(channels.SHARED_DATA_UPDATED),
          (payload, vars) => {
            const result =
              payload.sharedDataUpdated.id === vars.id &&
      ...
      localeAdded: {
        subscribe: (parent, args, { pubsub }) =>
          pubsub.asyncIterator(channels.LOCALE_ADDED),
      },
      routeRequested: {
        subscribe: (parent, args, { pubsub }) =>
          pubsub.asyncIterator(channels.ROUTE_REQUESTED),
      },
    },
  },
];
...
const paths = globby.sync(['./schema/*.js'], {
  cwd: __dirname,
  absolute: true,
});
paths.forEach(file => {
