...
exports.resolvers = {
  WidgetDefinition: {
    canAddMore: (definition, args, context) =>
      widgets.canAddMore(definition, context),
    count: (definition, args, context) => widgets.getCount(definition.id),
  },

  Widget: {
    definition: (widget, args, context) =>
      widgets.findDefinition(widget, context),
    prompts: (widget, args, context) =>
      widgets.getConfigPrompts(widget, context),
  },

  Query: {
    widgetDefinitions: (root, args, context) =>
      widgets.listDefinitions(context),
  ...
  Mutation: {
    ...
    widgetConfigOpen: (root, { id }, context) =>
      widgets.openConfig({ id }, context),
    widgetConfigSave: (root, { id }, context) =>
      widgets.saveConfig({ id }, context),
    widgetConfigReset: (root, { id }, context) =>
      widgets.resetConfig({ id }, context),
  },
};
