...
exports.resolvers = {
  Task: {
    ...
    plugin: (task, args, context) =>
      plugins.findOne({ id: task.pluginId, file: task.path }, context),
  ...
  Mutation: {
    ...
    taskSaveParameters: (root, { id }, context) =>
      tasks.saveParameters({ id }, context),
    taskRestoreParameters: (root, { id }, context) =>
      tasks.restoreParameters({ id }, context),
  },

  Subscription: {
    taskChanged: {
      subscribe: (parent, args, { pubsub }) =>
        pubsub.asyncIterator(channels.TASK_CHANGED),
    },
    taskLogAdded: {
      subscribe: withFilter(
        (parent, args, { pubsub }) =>
          pubsub.asyncIterator(channels.TASK_LOG_ADDED),
