...
exports.resolvers = {
  Folder: {
    ...
    isPackage: (folder, args, context) =>
      folders.isPackage(folder.path, context),
    isVueProject: (folder, args, context) =>
      folders.isVueProject(folder.path, context),
    isBorProject: (folder, args, context) =>
      folders.isBorProject(folder.path, context),
    favorite: (folder, args, context) =>
      folders.isFavorite(folder.path, context),
  },
  ...
  Mutation: {
    ...
    folderOpenParent: (root, args, context) =>
      folders.openParent(cwd.get(), context),
    folderSetFavorite: (root, args, context) =>
      folders.setFavorite(
        {
          file: args.path,
