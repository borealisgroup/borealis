...
test('build as lib', async () => {
  ...
  const { stdout } = await project.run('bor-cli-service build --target lib --name testLib src/components/HelloWorld.vue')
...
test('build as lib (js)', async () => {
  ...
  await project.write('src/main.js', `
    ...
    export const bar = 2
  `)
  const { stdout } = await project.run('bor-cli-service build --target lib --name testLib src/main.js')
  expect(stdout).toMatch('Build complete.')
...
test('build as lib with --filename option', async () => {
  ...
  await project.write('src/main.js', `
      ...
      export const bar = 2
    `)
  const { stdout } = await project.run('bor-cli-service build --target lib --name testLib --filename test-lib src/main.js')
  expect(stdout).toMatch('Build complete.')
...
test('build as lib without --name and --filename options (default to package name)', async () => {
  ...
  await project.write('src/main.js', `
      ...
      export const bar = 2
    `)
  const { stdout } = await project.run('bor-cli-service build --target lib src/main.js')
  expect(stdout).toMatch('Build complete.')
...
test('build as lib with --inline-vue', async () => {
  ...
  const { stdout } = await project.run('bor-cli-service build --target lib --inline-vue --name testLib src/main-lib.js')
  expect(stdout).toMatch('Build complete.')
