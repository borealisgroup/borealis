...
test('build as lib', async () => {
  ...
  const { stdout } = await project.run(
    'bor-cli-service build --target lib --name testLib src/components/HelloWorld.vue'
  );
...
test('build as lib (js)', async () => {
  ...
  const { stdout } = await project.run(
    'bor-cli-service build --target lib --name testLib src/main.js'
  );
  expect(stdout).toMatch('Build complete.');
...
test('build as lib with --filename option', async () => {
  ...
  const { stdout } = await project.run(
    'bor-cli-service build --target lib --name testLib --filename test-lib src/main.js'
  );
  expect(stdout).toMatch('Build complete.');
...
test('build as lib without --name and --filename options (default to package name)', async () => {
  const project = await create(
    'build-lib-no-name-and-filename-option',
    defaultPreset
  );
  await project.write(
    'package.json',
  ...
  const { stdout } = await project.run(
    'bor-cli-service build --target lib src/main.js'
  );
  expect(stdout).toMatch('Build complete.');
...
test('build as lib without --name and --filename options (default to package name, minus scope)', async () => {
  const project = await create(
    'build-lib-no-name-and-filename-option-with-scope',
    defaultPreset
  );
  await project.write(
    'package.json',
...
test('build as lib with --inline-vue', async () => {
  ...
  const { stdout } = await project.run(
    'bor-cli-service build --target lib --inline-vue --name testLib src/main-lib.js'
  );
  expect(stdout).toMatch('Build complete.');
