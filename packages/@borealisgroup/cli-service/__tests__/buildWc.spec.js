...
test('build as wc', async () => {
  ...
  const { stdout } = await project.run(
    `bor-cli-service build --target wc **/*.vue`
  );
  ...
  const childStyleCount = await page.evaluate(() => {
    return document
      .querySelector('build-wc-hello-world')
      .shadowRoot.querySelectorAll('style').length;
  });
...
test('build as single wc', async () => {
  ...
  const { stdout } = await project.run(
    `bor-cli-service build --target wc --name single-wc`
  );
  expect(stdout).toMatch('Build complete.');
  ...
  const styleCount = await page.evaluate(() => {
    return document
      .querySelector('single-wc')
      .shadowRoot.querySelectorAll('style').length;
  });
  expect(styleCount).toBe(2); // should contain styles from both app and child

  const h1Text = await page.evaluate(() => {
    return document.querySelector('single-wc').shadowRoot.querySelector('h1')
      .textContent;
  });
  expect(h1Text).toMatch('Welcome to Your Vue.js App');
});
...
test('build as wc with --inline-vue', async () => {
  ...
  const { stdout } = await project.run(
    'bor-cli-service build --target wc --inline-vue --name single-wc src/main-wc.js'
  );
  expect(stdout).toMatch('Build complete.');
