...
test('loading plugins from package.json', () => {
  mockPkg({
    devDependencies: {
      bar: '^1.0.0',
      '@borealisgroup/cli-plugin-babel': '^4.0.0',
...
test('api: assertVersion', () => {
  const plugin = {
    ...
    apply: api => {
      ...
      expect(() => api.assertVersion(4.1)).toThrow(
        'Expected string or integer value'
      );
      expect(() => api.assertVersion('^100')).toThrow(
        'Require @borealisgroup/cli-service "^100"'
      );
    },
  };
...
test('api: configureWebpack preserve ruleNames', () => {
  const service = createMockService([
    {
      id: 'babel',
      apply: require('@borealisgroup/cli-plugin-babel'),
    },
    {
      id: 'test',
      apply: api => {
