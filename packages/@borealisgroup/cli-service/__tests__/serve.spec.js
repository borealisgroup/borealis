...
test('serve with router', async () => {
  const project = await create('e2e-serve-router', {
    ...defaultPreset,
    plugins: {
  ...
  await serve(
    ...
    async ({ page, helpers }) => {
      ...
      expect(
        await helpers.hasClass('a[href="#/"]', 'router-link-exact-active')
      ).toBe(true);
      expect(
        await helpers.hasClass('a[href="#/about"]', 'router-link-exact-active')
      ).toBe(false);
...
test('serve with legacy router option', async () => {
  const project = await create('e2e-serve-legacy-router', {
    ...defaultPreset,
    router: true,
...
test('serve with legacy vuex option', async () => {
  const project = await create('e2e-serve-legacy-vuex', {
    ...defaultPreset,
    vuex: true,
  });
...
test('dart sass', async () => {
  ...
  await project.run('bor-cli-service build');
});
