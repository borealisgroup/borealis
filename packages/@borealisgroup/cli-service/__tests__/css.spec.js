...
const findOptions = (config, lang, _loader, index) => {
  const rule = findRule(config, lang, index);
  const use = rule.use.find(({ loader }) =>
    loader.includes(`${_loader}-loader`)
  );
  return use.options || {};
};
...
test('default loaders', () => {
  ...
  LANGS.forEach(lang => {
    ...
    expect(findLoaders(config, lang)).toEqual(
      ['vue-style', 'css', 'postcss'].concat(loader)
    );
...
test('production defaults', () => {
  ...
  LANGS.forEach(lang => {
    const loader = lang === 'css' ? [] : LOADERS[lang];
    expect(findLoaders(config, lang)).toEqual(
      [extractLoaderPath, 'css', 'postcss'].concat(loader)
    );
    expect(findOptions(config, lang, 'postcss').plugins).toBeFalsy();
    expect(findOptions(config, lang, 'css')).toEqual({
      sourceMap: false,
...
test('deprecate `css.modules` option', () => {
  ...
  expect(
    logs.warn.some(([msg]) =>
      msg.match('please use "css.requireModuleExtension" instead')
    )
  ).toBe(true);
...
test('css.extract', () => {
  ...
  LANGS.forEach(lang => {
    ...
    expect(findLoaders(config2, lang)).toEqual(
      ['vue-style', 'css', 'postcss', 'postcss'].concat(loader)
    );
...
test('css.loaderOptions', () => {
  ...
  expect(findOptions(config, 'scss', 'sass').sassOptions).not.toHaveProperty(
    'indentedSyntax'
  );
  expect(findOptions(config, 'sass', 'sass')).toMatchObject({
    prependData,
...
test('scss loaderOptions', () => {
  ...
  expect(findOptions(config, 'sass', 'sass')).not.toHaveProperty(
    'webpackImporter'
  );
});
