...
test('modern mode', async () => {
  ...
  const { stdout } = await project.run('bor-cli-service build --modern');
  ...
  expect(files.some(f => /^chunk-vendors-legacy\.\w{8}\.js$/.test(f))).toBe(
    true
  );

  // arrow function should be reserved in the modern build
  const app = await project.read(
    `dist/js/${files.find(f => /^app\.\w{8}\.js$/.test(f))}`
  );
  expect(app).toMatch(/=>/);
  const legacyApp = await project.read(
    `dist/js/${files.find(f => /^app-legacy\.\w{8}\.js$/.test(f))}`
  );
  ...
  const { stdout: stdout2 } = await project.run(
    'bor-cli-service build --modern'
  );
...
test('no-unsafe-inline', async () => {
  ...
  const { stdout } = await project.run(
    'bor-cli-service build --modern --no-unsafe-inline'
  );
  expect(stdout).toMatch('Build complete.');
