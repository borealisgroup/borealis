...
test('build as lib with formats commonjs and umd', async () => {
  const { stdout } = await project.run(
    'bor-cli-service build --target lib --formats commonjs,umd --name testLib src/components/HelloWorld.vue'
  );
  expect(stdout).toMatch('Build complete.');
...
test('build as lib with format umd-min', async () => {
  const { stdout } = await project.run(
    'bor-cli-service build --target lib --formats umd-min --name testLib src/components/HelloWorld.vue'
  );
  expect(stdout).toMatch('Build complete.');
...
test('build as lib with unknown formats throws an error', async () => {
  try {
    await project.run(
      'bor-cli-service build --target lib --formats umd,x,y --name testLib src/components/HelloWorld.vue'
    );
  } catch (e) {
