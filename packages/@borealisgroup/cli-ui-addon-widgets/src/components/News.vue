<template>
  ...
  >
    ...
    <div v-else-if="error" class="error vue-ui-empty">
    ...
    <div v-else class="panes">
      ...
      <transition>
        <div v-if="selectedItem" class="item-details">
          ...
          <NewsItemDetails v-if="selectedItem" :item="selectedItem" />
        </div>
...
export default {
  ...
  methods: {
    async fetchFeed(force = false) {
      ...
      try {
        const { results, errors } = await this.$callPluginAction(
          'org.vue.widgets.actions.fetch-news',
          {
