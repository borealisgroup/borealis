...
const getAbsolutePolyfill = mod => {
  ...
  return new RegExp(
    `import "${['.*node_modules', 'core-js', 'modules', mod].join(
      `[\\${path.sep}]+`
    )}`
  );
};
...
beforeEach(() => {
  process.env.VUE_CLI_ENTRY_FILES = JSON.stringify([
    path.join(process.cwd(), 'test-entry-file.js'),
  ]);
});
...
test('async/await', () => {
  ...
  expect(code).toMatch(
    /import _asyncToGenerator from ".*runtime-corejs3\/helpers\/esm\/asyncToGenerator\"/
  );
});
