...
const create = require('@borealisgroup/cli/lib/create');
...
test('fetching local preset with prompts and generator', async () => {
  ...
  const testFile = await fs.readFile(
    path.resolve(cwd, name, 'test.js'),
    'utf-8'
  );
  expect(testFile).toBe('true');

  const pkg = require(path.resolve(cwd, name, 'package.json'));
  expect(pkg.devDependencies).toHaveProperty('@borealisgroup/cli-plugin-babel');
});
