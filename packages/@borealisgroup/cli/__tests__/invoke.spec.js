...
const baseESLintConfig = {
  ...require('@vue/cli-plugin-eslint/eslintOptions').config({
    hasPlugin: () => false,
  }),
  rules: {
    'no-console': 'off',
...
async function assertUpdates(project) {
  ...
  expect(updatedPkg.scripts.lint).toBe('bor-cli-service lint');
...
extends:
  ...
  await project.run(
    `${require.resolve('../bin/vue')} invoke eslint --config airbnb`
  );
