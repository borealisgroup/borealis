...
const renderFile = (name, data, ejsOptions) => {
...
class GeneratorAPI {
  ...
  get cliServiceVersion() {
    ...
    const servicePkg = loadModule(
      '@borealisgroup/cli-service/package.json',
  ...
  //   throw new Error(
  //     `Require @borealisgroup/cli-service "${range}", but was loaded with "${this.cliServiceVersion}".`
  //   );
  // }
  ...
  render(source, additionalData = {}, ejsOptions = {}) {
    ...
    if (isString(source)) {
      ...
      this._injectFileMiddleware(files => {
        const data = this._resolveData(additionalData);
        for (const targetPath of Object.keys(source)) {
