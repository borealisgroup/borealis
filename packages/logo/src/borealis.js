...
const getViewBox = (shape, name, tagline, margin) => {
  let x = 0;
  let y = 0;
  let width = 708.7;
  let height = 223.9;

  if (shape && name && !tagline) {
    height = 120;
  } else if (shape && !tagline && !name) {
    width = 116;
    height = 116;
  ...
  if (margin) {
    const marginUnit = 64; // width/height of B in the borealis logo. See guidelines
    x -= marginUnit;
    y -= marginUnit;
    width += marginUnit * 2;
    height += marginUnit * 2;
...
const borealis = ({
  ...
  const viewboxDimensions = getViewBox(shape, name, tagline, margin);

  const backgroundSvg = () => {
    const fullDimensions = `x="${viewboxDimensions.x}" y="${viewboxDimensions.y}" width="${viewboxDimensions.width}" height="${viewboxDimensions.height}"`;
    const x1 = -13.7201;
    const x2 = viewboxDimensions.width - x1;
    const y1 = 130.5376;
    const y2 = viewboxDimensions.width - y1;
    switch (background) {
      case 'TRANSPARENT':
        return '';
      case 'SOLID':
        return `<rect fill='#002d5a' ${fullDimensions}/>`
      case 'GRADIENT':
        return `
          <linearGradient id="SVGID_1_" gradientUnits="userSpaceOnUse" x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}">
            <stop  offset="0" style="stop-color:#002D5A"/>
            <stop  offset="0.25" style="stop-color:#005D9A"/>
            <stop  offset="0.6" style="stop-color:#0099D2"/>
            <stop  offset="1" style="stop-color:#82CFF5"/>
          </linearGradient>
          <rect style="fill:url(#SVGID_1_);" ${fullDimensions}/>
        `
      default:
        return '';
    }
  ...
  let svgString = `<svg
  viewBox="${viewboxDimensions.combined}"
  >
  ...
  if (color) {
    svgString = svgString.replace(/fill="\S+"/g, `fill="${color}"`);
  }
  return svgString;
};
